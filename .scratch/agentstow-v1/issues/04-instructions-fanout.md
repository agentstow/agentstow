...
- [x] Absent instructions file → symlink created per registry mechanism, per target
- [x] `CLAUDE.md` without the import line → line added, rest of the file byte-identical; with the line → no-op
- [x] Roo's rules directory receives the `AGENTS.md` link
- [x] Foreign-content instructions file → no write occurs; `status` shows a conflict with a remediation hint naming the occupying tool where known
- [x] Verify-at-build rows (oh-my-pi's probe path, Windsurf's global rules file) confirmed against the installed agent or corrected in the registry
